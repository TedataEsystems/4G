...
@Component({
  selector: 'app-pc-connect-req',
  templateUrl: './pc-connect-req.component.html',
  styleUrls: ['./pc-connect-req.component.css'],
  animations: [
    trigger('fadeInOut', [
...
export class PcConnectReqComponent implements OnInit {
 searchKey:string ='' ;
    ...
    )
  {
    ...
    this.titleService.setTitle('4G | PC-Connect-Request');
...
displayedColumns2: string[] = ['id','action',"sim", 'customerConnectionRequestType','customerName',
 'ecrmNumberSample','ipAllocationId','forecastId','forecastNumber','noOfSIMs'
,'confirmationSummaryApprovedFromCSTId','destinationIpOfCustomer',
'requestedQuota','billingApprovalConfirmationId','customerTestingTool','vrfId','oldECRMNumber','vrfDetails',
'customerNeedsSpecificPortOrAllowAnyId','customerNeedOneWayOrTwoWaysDirectionsId','ipPool',
'confirmedThatIpPoolNotDuplicatedId','casePeriority','contracttypeId','apnName',
 "currentstatusId",'usedTestSimDataSerialId','generalComment',
'creationDate','modificationDate','createdBy','modifyiedBy','createdByTeam','modifyiedByTeam'
 ...
 pageIn=0;
 previousSizedef=25;
simflag=true;reqtype='pc connect';
adminflag=true;
...
lastdir:string='asc';
 ...
 this.lastcol=sort.active;this.lastdir=sort.direction;
    ...
    onCreate(){
  this.router.navigate(['/pcconnectreqdetail'] );      
    }

    onEdit(r:any){
    
  this.router.navigate(['/pcconnectreqdetail'],{ queryParams: {id: r.id}});
   
    }
    
    onEditReq(r:any){
      sessionStorage.setItem('simkey','pc');
