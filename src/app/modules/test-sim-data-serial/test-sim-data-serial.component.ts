...
@Component({
  selector: 'app-test-sim-data-serial',
  templateUrl: './test-sim-data-serial.component.html',
  styleUrls: ['./test-sim-data-serial.component.css']
})
export class TestSimDataSerialComponent implements OnInit {
...
delpic:any;
  ...
  param1='';settingtype='Test Sim Serial';
  constructor(private titleService:Title,private pickser:TestSimDataSerialService
    ...
    // Create 100 users
    this.titleService.setTitle('4G | Test Sim Serial');
  ...
  ngOnInit() {
    ...
    this.service.onMessage().subscribe(x=>{
      this.pickser.delTestSimDataSerial(this.delpic.id).subscribe(
        res=>{
...
this.dataSource._updateChangeSubscription()
 ...
 if(this.simflag){
  ...
  getRequestdata(attr:any ){
    ...
    this.pickser.getTestSimDataSerial().subscribe(res=>{
      this.loading = false;
...
//this.router.navigate(['/Simdetail'],{ queryParams: {id: r.id}});
  ...
  onSubmit() {
   ...
   debugger;
      ...
      this.pickser.addTestSimDataSerial(listval).subscribe((res)=>{
         this.isDisable=false;
    ...
    this.dataSource.paginator = this.paginator as MatPaginator;
     ...
     listval.id=Number(this.form.value.id);
      this.pickser.editTestSimDataSerial(listval).subscribe((res)=>{
         this.isDisable=false;     
        if(res.status==true)    {         
