...
@Component({
  selector: 'app-apnworked-from1st-test',
  templateUrl: './apnworked-from1st-test.component.html',
  styleUrls: ['./apnworked-from1st-test.component.css']
})
export class APNWorkedFrom1stTestComponent implements OnInit {
...
delpic:any;
  ...
  param1='';settingtype='APN Worked From 1st Test';
  constructor(private titleService:Title,private pickser:APNWorkedFrom1stTestService
    ...
    // Create 100 users
    this.titleService.setTitle('4G | APN Worked from 1st test');
  ...
  ngOnInit() {
    ...
    this.service.onMessage().subscribe(x=>{
      this.pickser.delAPNWorkedFrom1stTest(this.delpic.id).subscribe(
        res=>{
...
this.dataSource._updateChangeSubscription()
 ...
 if(this.simflag){
  ...
  getRequestdata(attr:any ){
    ...
    this.pickser.getAPNWorkedFrom1stTest().subscribe(res=>{
      this.loading = false;
...
//this.router.navigate(['/Simdetail'],{ queryParams: {id: r.id}});
  ...
  onSubmit() {
   ...
   debugger;
      ...
      this.pickser.addAPNWorkedFrom1stTest(listval).subscribe((res)=>{
         this.isDisable=false;
    ...
    this.dataSource.paginator = this.paginator as MatPaginator;
     ...
     listval.id=Number(this.form.value.id);
      this.pickser.editAPNWorkedFrom1stTest(listval).subscribe((res)=>{
         this.isDisable=false;     
        if(res.status==true)    {         
