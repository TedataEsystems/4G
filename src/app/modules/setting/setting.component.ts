...
export class SettingComponent implements OnInit {
  ...
  versions=["forecast","vrf","ipallocation",'status',"billing approval confirmation","apn","problem","core comment","epm comment","consultancy comment",'case periority','apn type','sim status'];
...
delpic:any;
  ...
  param1='';settingtype=''
  constructor(private titleService:Title,private pickser:PickListSerService
    ...
    // Create 100 users
    //this.titleService.setTitle('4G | Settings');

    this.route.queryParams.subscribe((params:any) => {
    debugger;
      this.param1 = params['val'];    
      if(this.param1!=undefined){
   this.settingtype=this.param1;
   this.titleService.setTitle('4G | ' + this.settingtype);
   this.getRequestdata('');
      }
      else   
      {
  ...
  ngOnInit() {
   // this.getRequestdata("");  
    this.service.onMessage().subscribe(x=>{
      this.pickser.delSimData(this.delpic.id).subscribe(
        res=>{
...
this.dataSource._updateChangeSubscription()
 ...
 if(this.simflag){
  ...
  apply(filterValue:string) {
    ...
    this.pickListTab=[];
  this.pickList?.forEach(x=>{
    if(x.attributeName?.toLocaleLowerCase()==filterValue.toLocaleLowerCase())
    this.pickListTab?.push(x);
  })
...
//this.router.navigate(['/Simdetail'],{ queryParams: {id: r.id}});
  ...
  onSubmit() {
   ...
   debugger;
      ...
      listval.attributeName=this.param1; 
      ...
      this.pickser.addReqest(listval).subscribe((res)=>{
         this.isDisable=false;
         
      if(res.status==true)    {
        var pick:PickList=new PickList();
        pick.attributeName=res.data?.attributeName;
        pick.id=res.data?.id;
    ...
    this.dataSource.paginator = this.paginator as MatPaginator;
     ...
     listval.id=Number(this.form.value.id);
      this.pickser.editReqest(listval).subscribe((res)=>{
         this.isDisable=false;
