...
delpic:any;
  ...
  param1='';settingtype='Current Status';
  constructor(private titleService:Title,private pickser:CurrentStatusService
    ...
    // Create 100 users
    this.titleService.setTitle('4G | Current Status');
  ...
  ngOnInit() {
    ...
    this.service.onMessage().subscribe(x=>{
      this.pickser.delCurrentStatus(this.delpic.id).subscribe(
        res=>{
...
this.dataSource._updateChangeSubscription()
 ...
 if(this.simflag){
  ...
  getRequestdata(attr:any ){
    ...
    this.pickser.getCurrentStatus().subscribe(res=>{
      this.loading = false;
...
//this.router.navigate(['/Simdetail'],{ queryParams: {id: r.id}});
  ...
  onSubmit() {
   ...
   debugger;
      ...
      this.pickser.addCurrentStatus(listval).subscribe((res)=>{
         this.isDisable=false;
    ...
    this.dataSource.paginator = this.paginator as MatPaginator;
     ...
     listval.id=Number(this.form.value.id);
      this.pickser.editCurrentStatus(listval).subscribe((res)=>{
         this.isDisable=false;     
        if(res.status==true)    {         
